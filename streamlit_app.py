...
# --- 4. قاعدة البيانات (تمت إضافة الحالة: متاح/محجوز) ---
DATA = {
    "أطباء": [
        {"n": "د. علي الركابي", "s": "قلبية وباطنية", "a": "الحارثية", "stars": "⭐⭐⭐⭐⭐", "dist": "1.2 كم", "phone": "07701234567", "rank": "الأكثر خبرة", 
         "slots": [("03:00 PM", True), ("04:30 PM", False), ("06:00 PM", True), ("07:30 PM", False)]},
        {"n": "د. سارة الجبوري", "s": "باطنية عامة", "a": "المنصور", "stars": "⭐⭐⭐⭐", "dist": "3.5 كم", "phone": "07801112223", "rank": "الأقرب إليك", 
         "slots": [("03:30 PM", True), ("05:00 PM", True), ("06:30 PM", False), ("08:00 PM", True)]},
        {"n": "د. مريم القيسي", "s": "مفاصل وتأهيل", "a": "الكرادة", "stars": "⭐⭐⭐⭐⭐", "dist": "5.0 كم", "phone": "07901231234", "rank": "استشاري", 
         "slots": [("04:00 PM", False), ("05:30 PM", True), ("07:00 PM", True), ("08:30 PM", False)]}
    ]
}
...
# --- المرحلة 2: التحليل والعرض ---
    ...
    if st.session_state.get('diag_ready'):
        ...
        st.write("### 👨‍⚕️ الأطباء المتاحون (دوام 3:00 - 8:00):")
        for d in DATA["أطباء"]:
            st.markdown(f'''
                <div class="main-card">
                    ...
                    <div style="margin-top:8px; margin-bottom:10px;">
                        ...
                        <span style="font-size:13px; color:#bbb;">{d["stars"]} | 📍 {d["dist"]}</span>
                    </div>
                </div>
            ''', unsafe_allow_html=True)
            
            # عرض الأوقات بشكل أفقي
            ...
            for i, (time, is_available) in enumerate(d['slots']):
                with cols[i]:
                    if is_available:
                        ...
                        st.button(f"🔒 {time}", key=f"{d['n']}-{time}", disabled=True)
    ...
    st.markdown(f'''
        <div class="final-receipt">
            ...
            <div class="receipt-body">
                ...
                <p style="border-bottom: 1px solid #222;">🕒 <b>الوقت:</b> اليوم {st.session_state.final_time}</p>
                <p style="border-bottom: 1px solid #222;">📍 <b>العنوان:</b> {st.session_state.selected_doc['a']}</p>
